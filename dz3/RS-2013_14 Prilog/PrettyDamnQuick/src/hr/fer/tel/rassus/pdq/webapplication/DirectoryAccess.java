...
public class DirectoryAccess {
    ...
    public static void main(String[] args) {
        ...
        final float p_BP = 0.2f;
        final float p_IM_inc = 0.05f;
        final float p_IM_max = 0.5f;

        final float S_ZZ = 0.001f;
        final float S_PO = 0.001f;

        final float S_AP = 0.3f;
        final float S_BP = 4.5f;
        final float S_IM = 0.5f;

        final float L = 0.5f;

        System.out.print("p_IM\t");
        ...
        for (float p_IM = p_IM_inc; p_IM < p_IM_max + p_IM_inc; p_IM += p_IM_inc) {
